...
memisc::Table(AMERLATINA$Países.x)
...
colnames(AMERLATINA)
summary(AMERLATINA$ConfInterpessoal)
...
C = 1.466247 #Variância do intercepto do modelo com todas as variáveis
N = 1.548364 #Variância do intercepto do modelo sem variáveis
X = C/N 
X= 0.9469653
Y = 1 - X
#[1] 0.05303469
...
table(AMERLATINA$q1tc_r)
table(AMERLATINA$Sexo)
AMERLATINA$SexoN <- AMERLATINA$q1tc_r
...
AMERLATINA$Sexo <- as.numeric(AMERLATINA$Sexo)
...
# [1] 4.888026 7.193771 4.448726 6.754472 3.320006 5.625751 3.652589 5.958334 6.079661 6.079661
# attr(,"label")
# [1] "Predicted values"
...
# média antiga de tolerância por país - fazer o mesmo para o ive
library(tidyverse)

AMERLATINA$Países.x

mediaTolPais <- AMERLATINA %>%
  group_by(Países.x) %>%
  summarise(Média_Tolerância = mean(MedToler))
...
mediaTolPais23 <- AMERLATINA %>%
  group_by(Países.x) %>%
  summarise(Média= mean(TolerHomo))

# Exibindo a média
print(mediaTolPais23)

summary(AMERLATINA$TolerHomo)

# Verificar se há valores ausentes na variável de interesse
sum(is.na(AMERLATINA$TolerHomo))

# Calcular a média para cada país
media_por_pais <- AMERLATINA %>%
  group_by(Países.x) %>%
  summarise(Média = mean(TolerHomo, na.rm = TRUE))
